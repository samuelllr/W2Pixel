...
function wDiamond( x , y , tam , ajeitar ){
    ...
    //pega o centro width
    var centroW = Math.floor(tam/2);
    //pega o centro height
    var centroH = Math.floor(tam/2);

    //cima left
    wLinePath([centroW+x-fix1,y,x,centroH+y]);
    //cima right
    wLinePath([centroW+x,y,(tam-1)+x,centroH+y]);
    //baixo left
    wLinePath([x,centroH+y,centroW+x,(tam-1)+y]);
    //baixo right
    wLinePath([(tam-1)+x,centroH+y,centroW+x,(tam-1)+y]);

    //==============preenchimento

        //3x3(menor tamanho)
            if(tam==3){
                wPoint(centroW,centroH);
            }
        //=================
    ...
    var countSpaceW = 0;
    //ponta Y1
    var tamLinePathsY1 = fix3 ;
    //ponta Y2
    var tamLinePathsY2 = tam-1 ;
    //====matematica
    for( spaceW=centroW+fix2; spaceW<tam ; spaceW++ ){
        //matematica
        countSpaceW++;
        tamLinePathsY1++;
        tamLinePathsY2--;
        ...
        //right
        wLinePath([centroW+countSpaceW+x,tamLinePathsY1+y,centroW+countSpaceW+x,tamLinePathsY2+y]);
        //left
        wLinePath([centroW-countSpaceW+x,tamLinePathsY1+y,centroW-countSpaceW+x,tamLinePathsY2+y]);
        //centre
        wLinePath([centroW+x,tamLinePathsY1-1+y,centroW+x,tamLinePathsY2+1+y]);
...
//===============SALVA CANVAS TO IMAGE====== NO FUNCTION ==================
...
document.addEventListener("keypress",function(){
    ...
    var code = e.which || e.keyCode;
    // CTRL + M
    if( ( e.which || e.keyCode == 77 )&& e.ctrlKey ){ 
        code = "ctrlm" ;
        if( code == "ctrlm" ){
            ...
            if ( modSVGCANVAS == "svg" ){
                ...
                //SALVA EM IMG SVG
                //DTA IMG ESP
                var s = new XMLSerializer().serializeToString(esp);
